...
const notesGraphStore = useNotesGraphStore();
...
<template>
	<div class="absolute top-0 bg-gray-200 p-2 box-border flex " :style="'left:'+x.left+'px; width:'+x.width+'px'">
		<input type="text" ref="input_elem" v-model="uiStateStore.cur_search" 
			class="flex-grow border border-gray-700 "
			:class="[ notesGraphStore.last_search_error ? 'bg-red-100' : 'bg-white']" />
		<span class="ml-2">
